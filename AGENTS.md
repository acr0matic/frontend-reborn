...
В `.agents/skills/` лежат проектные скиллы (автообнаружение, коммитятся в git):

| Скилл | Когда использовать |
| --- | --- |
| `/new-page` | Создание новой страницы — скаффолд по методологии |
| `/verify-page` | Приёмка свёрстанной страницы — линтеры, сборка, браузер |
| `accessibility`, `performance`, `core-web-vitals`, `best-practices`, `web-quality-audit` | Внешние стандарты качества (web-quality-skills) |

При задачах «создать страницу» и «проверь вёрстку» вызывай соответствующий скилл вместо импровизации.

## 17. Алгоритм работы и приёмка
